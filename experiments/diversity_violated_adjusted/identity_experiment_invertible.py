...
from invertible_network_utils import construct_invertible_mlp, construct_invertible_flow
...
d_intermediate = 3  # Intermediate dimension (after g, before f) - Invertible network outputs 3D
...
# Initialize Invertible Neural Network ONCE - reused for all experiments (3D → 3D)
g = construct_invertible_mlp(n=d_input, n_layers=3, act_fct="leaky_relu").to(device)
g = torch.compile(g)  # Compile for 20-40% speedup since g is never updated
# Warm up compilation
_ = g(torch.randn(100, 3, device=device))
print(f"🔧 Initialized and compiled shared Invertible MLP: {d_input}D → {d_intermediate}D")
...
def train_model(constraint_ratio, run_id=0, verbose=False):
    ...
    # Initialize fresh SphericalEncoder for each experiment (f is reinitialized)
    ...
    objective = InfoNceLossAdjusted(tau)
    # Composition: sphere data -> g (Invertible MLP 3D→3D) -> f (SphericalEncoder 3D→3D) 
...
results_dir = "experiment_results_invertible"
os.makedirs(results_dir, exist_ok=True)
results_file = os.path.join(results_dir, f"constraint_ratio_experiment_invertible_{timestamp}.json")
summary_file = os.path.join(results_dir, f"constraint_ratio_summary_invertible_{timestamp}.txt")

# Run quick profiling first
profile_bottlenecks()

print("🚀 Starting Constraint Ratio Experiment with Invertible Neural Network (5 runs per ratio)")
...
print(f"Architecture: 3D sphere → Invertible MLP({d_input}→{d_intermediate}) → SphericalEncoder({d_intermediate}→{d_output})")
print(f"Training setup: f (SphericalEncoder) reinitialized each run, g (Invertible MLP) shared across all {len(constraint_ratios) * n_runs} experiments")
...
existing_files = [f for f in os.listdir(results_dir) if f.startswith("constraint_ratio_experiment_invertible_") and f.endswith(".json")]
if existing_files:
...
experiment_metadata = {
    ...
    "n_runs": n_runs,
    "architecture": f"3D sphere → Invertible MLP({d_input}→{d_intermediate}) → SphericalEncoder({d_intermediate}→{d_output})",
    "training_setup": "f (SphericalEncoder) reinitialized each run, g (Invertible MLP) shared across all experiments",
    "results": []
}
...
for i, ratio in enumerate(constraint_ratios):
    ...
    with open(summary_file, 'w') as f:
        f.write(f"Constraint Ratio Experiment Results (Invertible Network)\n")
...
# 📊 RESULTS SUMMARY
print("\n" + "=" * 90)
print("📊 EXPERIMENT RESULTS SUMMARY - INVERTIBLE NETWORK (Mean ± Std over 5 runs)")
print("=" * 90)
...
axes[0, 0].set_title('Linear Disentanglement vs Constraint Ratio (Invertible)')
...
axes[0, 1].set_title('Permutation Disentanglement vs Constraint Ratio (Invertible)')
...
axes[1, 0].set_title('Angle Error vs Constraint Ratio (Invertible)')
...
axes[1, 1].set_title('Final Loss vs Constraint Ratio (Invertible)')
...
plt.tight_layout()
plot_filename = os.path.join(results_dir, f'constraint_ratio_experiment_invertible_plots_{timestamp}.png')
...
final_results_file = os.path.join(results_dir, f"constraint_ratio_final_results_invertible_{timestamp}.json")
