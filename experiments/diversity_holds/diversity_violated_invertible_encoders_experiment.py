...
# Experiment Configuration - diversity_violated with fixed dimensions + invertible encoders
...
def initialize_generative_processes_with_invertible_encoders():
    ...
    print("🏗️  Initialized 5 generative processes with invertible encoders and conditional sampling (d_fixed=1):")
...
def main():
    ...
    print("🚀 Starting Diversity Violated Experiment with Invertible Encoders")
    ...
    print(f"🔄 Using invertible encoders matched to each generative process")
    ...
    results_file = os.path.join(results_dir, f"diversity_violated_invertible_encoders_{timestamp}.csv")
    ...
    # Final summary
    print("\n" + "=" * 80)
    print("📊 FINAL RESULTS SUMMARY (Diversity Violated + Invertible Encoders)")
    print("=" * 80)
    ...
    # Save final summary
    summary_file = os.path.join(results_dir, f"diversity_violated_invertible_encoders_summary_{timestamp}.csv")
    ...
    averages_file = os.path.join(results_dir, f"diversity_violated_invertible_encoders_averages_{timestamp}.csv")
    averages_df.to_csv(averages_file, index=False)
    
    print("\n📊 PROCESS AVERAGES (Diversity Violated + Invertible Encoders):")
    print("-" * 80)
    for avg_data in averages_data:
        print(f"{avg_data['process_name']} ({avg_data['encoder_type']}) [d_fixed={avg_data['fixed_dims']}]:")
